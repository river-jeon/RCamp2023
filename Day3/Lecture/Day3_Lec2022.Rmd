...
sample() will return the elements sampled from the vector. The return value if **not** a list, which means if you need to do list operations, you need to first convert it to a list. 
```{r}
sample(x = 1:4, size = 2)
die <- 1:6
sample(x = die, size = 3)
...
But how do you know which argument names to use? 
...
We can use the **ifelse()** function to modify values in our dataframe. For example, we can create a new column in the dataframe called PopOver100 where its value if TRUE if the population is greater than 100 and FALSE otherwise. 
```{r eval = FALSE}
