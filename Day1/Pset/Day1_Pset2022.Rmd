...
6. Create a list l containing the vector numbers, and the letters A, B, C. Show values in l.
```{r}
### YOUR CODE GOES HERE
```
...
3. Create a vector with 12 integers from 1 to 12. Convert the vector to a 4x3 matrix B using matrix().
```{r}
### YOUR CODE GOES HERE
```
...
4. What is the dimension of the dataframe? How many observations and variables? What are the column names? 
```{r}
### YOUR CODE GOES HERE
```
...
Remember Ted asked you to solve for $$\hat{\beta} = (X^TX)^{-1}X^Ty$$ in Math Modeling. 
...
1. What is X and y in this dataframe? To be more specific, which columns should you include for X, and which column is the y? 
...
5. Now that you have all the elements, find $\hat{\beta} = (X^TX)^{-1}X^Ty$. You can do it step by step to make is easier to interpret. For example, find $(X^TX)^{-1}$ first, then find $(X^TX)^{-1}X^Ty$. 
```{r}
### YOUR CODE GOES HERE 
```
